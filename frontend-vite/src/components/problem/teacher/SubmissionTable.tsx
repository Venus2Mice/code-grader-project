            ...
            return (
              <TableRow key={submission.id} className="border-b-2 border-border">
                ...
                <TableCell className="text-right">
                  <span className={`font-black text-lg ${(submission.score ?? submission.cached_score ?? 0) >= 80 ? 'text-green-600' : (submission.score ?? submission.cached_score ?? 0) >= 50 ? 'text-yellow-600' : 'text-red-600'}`}>
                    {submission.score ?? submission.cached_score ?? 0}
                  </span>
