...
interface LanguageDropdownProps {
  ...
  lockedLanguage?: string;  // Show locked language tooltip
}

export function LanguageDropdown({ value, onChange, disabled = false, lockedLanguage }: LanguageDropdownProps) {
  const currentLanguage = languages.find(l => l.code === value) || languages[0];
  const isLocked = disabled || !!lockedLanguage;
  ...
  return (
    <div className="relative group">
      <button 
        ...
        aria-label={isLocked ? "Language locked by teacher" : "Select language"}
        disabled={isLocked}
        title={isLocked ? `🔒 Language locked to ${currentLanguage.name} by teacher` : "Select language"}
      >
      ...
      {/* Only show dropdown if not locked */}
      {!isLocked && (
